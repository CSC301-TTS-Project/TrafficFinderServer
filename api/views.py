...
import json
from .ddb_actions import get_route_segments, get_route_segment_ids, insert_segment_in_route_record, \
    update_segment_in_route_record, delete_segment_from_route_record
...
def getRoute(request):
    ...
    try:
        ...
        route_record = get_route_segment_ids(USER, route)
...
def insertNode(request):
    ...
    try:
        ...
        if segment_idx == 0:
            insert_segment_in_route_record(USER, segment_idx, route, [new_node])
            return JsonResponse(json.dumps(new_node))
        else:
            # route to new node
            segment_ids = get_route_segment_ids()
